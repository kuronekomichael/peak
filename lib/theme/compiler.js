  ...
  compile: function () {
    ...
    if (nodes = this.parser.parse()) {
      ...
      this.buffer('options = options || {};'
        + 'var ctx = context;'
        + 'var buf = [];');
      this.buffer('if (ctx && ctx.hasOwnProperty("block:Posts")) {'
        + '  ctx["block:Posts"].forEach(function(post){'
        + '    if (!post.hasOwnProperty("PostType")) return;'
        + '    var postType = post["PostType"].charAt(0).toUpperCase() + post["PostType"].slice(1);'
        + '    if (!post["block:" + postType]) post["block:" + postType] = true;'
        + '  });'
        + '}');
      this.visit(nodes);
